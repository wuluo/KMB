 ...
 *     json    [json串]
 *
 * 新增方法 getValues() [check成功,用来获取有效值数组]
 *
 * 注意事项
 *     入参避免tp的I方法接收
 *
...
class Validate
{
	...
	/**
	 * get the valid data where check success
	 *
	 * @author guoquan <1257669560@qq.com>
	 *
	 * @return array
	...
	public function getValues()
	{
		...
		foreach ($this->rule as $field => $rule) {
			$value = $this->getDataValue($this->data, $field);
			if (is_string($rule)) {
				$rule = explode('|', $rule);
			}
			if ($this->isKeyExists($field, $this->data)) {
				//non-present ignore the value
				if ('' === $value && !in_array('present', $rule)) {
					continue;
				}
				if (strpos($field, '.')) {
	...
	/**
	 * detect the existence of key
	 *
	 * @author guoquan <1257669560@qq.com>
	 * @param  string $key  支持key1.key2形式
	 * @param  array  $data
	 *
	 * @retrun bool
	 */
	protected function isKeyExists($key, array $data)
	{
		if (strpos($key, '.')) {
			list($name1, $name2) = explode('.', $key);
			if (!array_key_exists($name1, $data) ||
				(is_array($data[$name1]) && !array_key_exists($name2, $data[$name1]))
			) {
				return false;
			}
		} else {
		...
		if ($rules instanceof \Closure) {
			...
			foreach ($rules as $key => $rule) {
				if ($rule instanceof \Closure) {
					...
					if (0 === strpos($info, 'require') || 0 === strpos($info, 'present') || (!is_null($value) && '' !== $value)) {
						// 验证类型
